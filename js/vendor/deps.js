...
var require_d3_dispatch = __commonJS({
  "node_modules/d3-cloud/node_modules/d3-dispatch/dist/d3-dispatch.js"(exports, module) {
...
var require_d3_cloud = __commonJS({
  "node_modules/d3-cloud/index.js"(exports, module) {
    ...
    module.exports = function() {
      var size = [256, 256], text = cloudText, font = cloudFont, fontSize = cloudFontSize, fontStyle = cloudFontNormal, fontWeight = cloudFontNormal, padding = cloudPadding, spiral = archimedeanSpiral, words = [], timeInterval2 = Infinity, event = dispatch2("word", "end"), timer2 = null, random = Math.random, rotate = () => (~~(random() * 6) - 3) * 30, cloud2 = {}, canvas = cloudCanvas;
        ...
        while (dxdy = s2(t += dt)) {
          ...
          if (!bounds || collideRects(tag, bounds)) {
            if (!cloudCollide(tag, board, size[0])) {
              var sprite = tag.sprite, w = tag.width >> 5, sw = size[0] >> 5, lx = tag.x - (w << 4), sx = lx & 127, msx = 32 - sx, h = tag.y1 - tag.y0, x4 = (tag.y + tag.y0) * sw + (lx >> 5), last;
              for (var j = 0; j < h; j++) {
                last = 0;
                for (var i = 0; i <= w; i++) {
                  board[x4 + i] |= last << msx | (i < w ? (last = sprite[j * w + i]) >>> sx : 0);
                }
                x4 += sw;
              }
              return true;
    ...
    function cloudSprite(contextAndRatio, d, data, di) {
      if (d.sprite) return;
      var c6 = contextAndRatio.context, ratio = contextAndRatio.ratio;
      c6.clearRect(0, 0, (cw << 5) / ratio, ch / ratio);
      var x4 = 0, y4 = 0, maxh = 0, n = data.length;
      --di;
      while (++di < n) {
        d = data[di];
        c6.save();
        c6.font = d.style + " " + d.weight + " " + ~~((d.size + 1) / ratio) + "px " + d.font;
        const metrics = c6.measureText(d.text);
        ...
        if (y4 + h2 >= ch) break;
        c6.translate((x4 + (w2 >> 1)) / ratio, (y4 + (h2 >> 1)) / ratio);
        if (d.rotate) c6.rotate(d.rotate * RADIANS);
        c6.fillText(d.text, anchor, 0);
        if (d.padding) c6.lineWidth = 2 * d.padding, c6.strokeText(d.text, anchor, 0);
        c6.restore();
        d.width = w2;
        d.height = h2;
        d.xoff = x4;
      ...
      var pixels = c6.getImageData(0, 0, (cw << 5) / ratio, ch / ratio).data, sprite = [];
      while (--di >= 0) {
        d = data[di];
        ...
        for (var j = 0; j < h; j++) {
          for (var i = 0; i < w; i++) {
            var k2 = w32 * j + (i >> 5), m3 = pixels[(y4 + j) * (cw << 5) + (x4 + i) << 2] ? 1 << 31 - i % 32 : 0;
            sprite[k2] |= m3;
            seen |= m3;
          }
          if (seen) seenRow = j;
    ...
    function cloudCollide(tag, board, sw) {
      sw >>= 5;
      var sprite = tag.sprite, w = tag.width >> 5, lx = tag.x - (w << 4), sx = lx & 127, msx = 32 - sx, h = tag.y1 - tag.y0, x4 = (tag.y + tag.y0) * sw + (lx >> 5), last;
      for (var j = 0; j < h; j++) {
        last = 0;
        for (var i = 0; i <= w; i++) {
          if ((last << msx | (i < w ? (last = sprite[j * w + i]) >>> sx : 0)) & board[x4 + i]) return true;
        }
        x4 += sw;
      }
      return false;
    ...
    function rectangularSpiral(size) {
      var dy = 4, dx = dy * size[0] / size[1], x4 = 0, y4 = 0;
      return function(t) {
        var sign3 = t < 0 ? -1 : 1;
...
var import_d3_cloud = __toESM(require_d3_cloud(), 1);
var export_cloud = import_d3_cloud.default;
export {
  export_cloud as cloud,
  src_exports as d3,
